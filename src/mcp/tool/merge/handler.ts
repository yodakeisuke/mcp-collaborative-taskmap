...
import { MergeToolParameters, taskToMergeResponse } from './schema.js';
...
export const mergeEntryPoint = (args: MergeToolParameters): Promise<CallToolResult> => {
  return loadCurrentPlan()
    ...
    .andThen(plan => {
      ...
      const newStatus = PrTaskStatus.merged();
      ...
      // Update the plan with the merged task
    ...
    .match(
      ({ updatedTask, previousStatus }) => {
        const response = taskToMergeResponse(updatedTask, previousStatus);
        return toCallToolResult([nextAction(), JSON.stringify(response, null, 2)], false);
      },
      error => toCallToolResult([`Failed to merge task: ${error.message}`], true)
    );
};